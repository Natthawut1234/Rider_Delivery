...
class _RiderJobsPageState extends State<RiderJobsPage>
  ...
  Future<void> _initializeRider() async {
    try {
      ...
      await _orderController.initializeSocket();
  ...
  Future<void> _acceptOrder(Order order) async {
    try {
      ...
      if (success) {
        ...
        _showErrorSnackBar(
          'ไม่สามารถรับงานได้: ${_orderController.errorMessage}',
        );
      }
    } catch (e) {
  ...
  Future<void> _updateOrderStatus(Order order, String status) async {
    try {
      ...
      if (success) {
        ...
        _showErrorSnackBar(
          'ไม่สามารถอัปเดตสถานะได้: ${_orderController.errorMessage}',
        );
      }
    } catch (e) {
      Navigator.of(context).pop();
      ...
      child: Scaffold(
        ...
        body: Consumer<RiderControllerSocket>(
          builder: (context, controller, _) {
            ...
            // Show tabs even when no orders (will show empty state in each tab)
            return TabBarView(
              controller: _tabController,
              children: [
      ...
      return const Center(
        child: Column(
          ...
          children: [
            ...
            SizedBox(height: 8),
            Text('ลากลงเพื่อรีเฟรช', style: TextStyle(color: Colors.grey)),
          ],
        ),
      );
    }
        ...
        return Row(
          children: [
            Expanded(
              child: ElevatedButton(
                onPressed: () =>
                    _updateOrderStatus(order, 'arrived_at_customer'),
                style: ElevatedButton.styleFrom(
